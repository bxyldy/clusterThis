...
//          myInstAttrRefs.material = inst_gdp->addStringTuple (GA_ATTRIB_PRIMITIVE, "shop_vm_surface", 1);
...
//          myInstAttrRefs.pointMaterial = inst_gdp->addStringTuple (GA_ATTRIB_POINT, "shop_vm_surface", 1);
...
//          myInstMBAttrOffsets.material = mb_gdp->addStringTuple (GA_ATTRIB_PRIMITIVE, "shop_vm_surface", 1);
...
//          myInstMBAttrOffsets.pointMaterial = mb_gdp->addStringTuple (GA_ATTRIB_POINT, "shop_vm_surface", 1);
...
//          myPointAttrOffsets.material = inst_gdp->pointAttribs().getOffset ( "shop_materialpath", GB_ATTRIB_INDEX );
//    myPointAttrOffsets.material = inst_gdp->pointAttribs().getOffset ( "shop_vm_surface", GB_ATTRIB_INDEX );


//int texOffset = inst_gdp->findPointAttrib( "texname", GB_ATTRIB_INDEX );
//GB_Attribute *texAttr = inst_gdp->pointAttribs().find( "texname", GB_ATTRIB_INDEX );
...
//       static_cast<int>(ppt->getValue<int>(attrRef, 0))))
    ...
    //
    //    //GB_Attribute *myAttr =  inst_gdp->pointAttribs().find( "shop_vm_surface", GB_ATTRIB_INDEX );
...
// VM_Surface_Attr = inst_gdp->getAttribute(GEO_POINT_DICT, "shop_vm_surface");
// VM_Surface_Attr = inst_gdp->getAttribute(GEO_POINT_DICT, "shop_materialpath");
...
//     myInstAttrRefs.material = inst_gdp->addPrimAttrib ( "shop_vm_surface", sizeof ( UT_String ), GB_ATTRIB_INDEX, 0 );
...
inline void VRAY_clusterThis::setPointInstanceAttributes(GU_Detail *inst_gdp, GEO_Point *ppt)
{
    ...
    if (myInstAttrRefs.pointCd.isValid()) {
        attrVector3Handle.bind(myInstAttrRefs.pointCd.getAttribute());
        attrVector3Handle.set(inst_gdp->pointOffset(myInstanceNum), (const UT_Vector3)(myPointAttributes.Cd));
    }
...
//    orient = myGeoPrim->castAttribData<UT_Vector4> ( myInstAttrRefs.orient );
